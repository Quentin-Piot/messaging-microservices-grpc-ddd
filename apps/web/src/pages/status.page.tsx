...
const Indicator = ({ isUp }: { isUp: boolean }) => (isUp ?
  <List.Indicator mt={2} color="green.500"> <LuCheckCircle /></List.Indicator> :
  <List.Indicator mt={2} color="red.500"> <LuStopCircle /></List.Indicator>);
const Item = ({ isUp, children }: PropsWithChildren<{ isUp: boolean }>) => <List.Item>
  <Indicator isUp={isUp} />{children}</List.Item>;
...
const StatusPage = () => {
  const { data } = useQuery({ queryKey: ["health"], queryFn: getHealthStatus, refetchInterval: 5000 });
  return <DemoLayout>
      ...
      <Card.Body gap="2">
        ...
        <Card.Description>
          <List.Root gap="2" variant="plain" align="center">
            ...
            <Item isUp={isServiceUp(data, "user_service")}> User service </Item>
